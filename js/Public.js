...
$(document).ready(function(){
	...
	function wdchange(){
		if($(window).width()<1200 && $(window).width()>640){
			$('#ResponsiveLink').attr('href', 'css/ipad.css');
		}else if($(window).width()<=640 && $(window).width()>=320){
			$('#ResponsiveLink').attr('href', 'css/mobile.css');
		}else{
			$('#ResponsiveLink').attr('href','');
		}
	}	
	wdchange();
	$(window).resize(wdchange);
});
...
$(document).ready(function(){
	var wapNavxin = 0;
	$('.wapNavBtn').click(function(event){
		if(wapNavxin==0){
			$('.wapNav').stop().slideDown(500);
			wapNavxin = 1;
		}else{
			$('.wapNav').stop().slideUp(500);
			wapNavxin = 0;
		}
	});
	function wap_Nav(){
		$('.wapNav li p img').each(function(index,el){
			var xinhao = index;
			$(el).click(function(){
				if(xinhao==index){
					$(this).parent('p').siblings('div').stop().slideDown(500);
					xinhao = index+1;
				}else{
					$(this).parent('p').siblings('div').stop().slideUp(500);
					xinhao = index;
				}
			});
		});
